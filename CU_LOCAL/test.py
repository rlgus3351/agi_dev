...
def ensure_meta_tables():
    ...
    _exec_sql(
        """
        CREATE TABLE IF NOT EXISTS dev_kkh.tb_audio_metadata (
            audio_metadata_id SERIAL PRIMARY KEY,
    ...
    _exec_sql(
        """
        CREATE TABLE IF NOT EXISTS dev_kkh.tb_file_metadata (
...
def upsert_audio_metadata(payload: dict):
    ensure_meta_tables()
    # 기존 삭제 후 삽입(간단 전략)
    _exec_sql("DELETE FROM dev_kkh.tb_audio_metadata WHERE item_id = %s", (payload.get("item_id"),))
    return _exec_sql(
        """
        INSERT INTO dev_kkh.tb_audio_metadata(
            item_id, file_path, file_ext, file_size_mb, duration_seconds,
            sample_rate, channels, codec_name, bitrate, recorded_ts
        ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        """,
        (
            payload.get("item_id"), payload.get("file_path"), payload.get("file_ext"), payload.get("file_size_mb"),
            payload.get("duration_seconds"), payload.get("sample_rate"), payload.get("channels"),
            payload.get("codec_name"), payload.get("bitrate"), payload.get("recorded_ts")
        )
    )


def upsert_file_metadata(payload: dict):
    ensure_meta_tables()
    _exec_sql("DELETE FROM dev_kkh.tb_file_metadata WHERE item_id = %s", (payload.get("item_id"),))
    return _exec_sql(
        """
        INSERT INTO dev_kkh.tb_file_metadata(
            item_id, file_path, file_ext, file_size_mb, num_lines
        ) VALUES (%s,%s,%s,%s,%s)
        """,
        (
            payload.get("item_id"), payload.get("file_path"), payload.get("file_ext"), payload.get("file_size_mb"),
            payload.get("num_lines")
        )
    )
            ...
            for t in tasks:
                ...
                if not item_id:
                    item_id = create_new_item_and_get_id_generic(
                        patient_uuid, data_category="PD", data_type=dt, seq=seq, description=f"{dt} 업로드"
                    )
                ...
                try:
                    ...
                    if kind == "video":
                        ...
                        payload = {
                            ...
                            "codec_name": meta.get("codec_name"),
                            "bitrate": meta.get("bitrate"),
                            "recorded_ts": meta.get("creation_time"),
                            "data_category": "PD",
                        }
                        ok, msg = upsert_audio_metadata(payload)
                        ...
                        payload = {
                            ...
                            "file_size_mb": meta.get("file_size_mb"),
                            "num_lines": meta.get("num_lines"),
                            "data_category": "PD",
                        }
                        ok, msg = upsert_file_metadata(payload)

                    else:
                        # 기타 → 최소 정보만 저장 (파일경로/사이즈)
                        payload = {
                            "item_id": item_id,
                            "file_path": target,
                            "file_ext": (t["file_ext"] or "").lstrip(".") if t["file_ext"] else (os.path.splitext(target)[1].lstrip("." ) if target else None),
