...
OUTPUT_PATH = os.path.abspath("C://Users//user//Desktop//frameSecurity//data//output_cv.mp4")
KEY_JSON_PATH = os.path.abspath("C://Users//user//Desktop//frameSecurity//data//video_encrypted_keys.json")
...
def encrypt_roi(roi: np.ndarray):
    # NumPy → Torch로 변환 후 GPU 연산
    roi_tensor = torch.from_numpy(roi).cuda().float()
    roi_bytes = roi_tensor.cpu().byte().numpy().tobytes()
    random_key = uuid.uuid4().hex[:16].encode()  # 16 bytes key
    encrypted_roi = aes_encrypt(roi_bytes, random_key)
