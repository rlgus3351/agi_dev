...
#         print(r.text)
def upload_to_nas(sid, local_path: str, nas_folder="/mAGI/CNU_Data/VIDEO", max_retries: int = 2, retry_delay: int = 60):
    """NAS에 파일 업로드 (자동 폴더 생성 + 실패 시 재시도)"""
    url = f"{NAS_URL}/webapi/entry.cgi"
    filename = os.path.basename(local_path)
    ...
    while attempt <= max_retries:
        try:
            print(f"📤 NAS 업로드 시도 {attempt}/{max_retries}: {filename}")
            with open(local_path, "rb") as f:
                files = {"file": (filename, f, "application/octet-stream")}
                r = requests.post(url, params=params, data=data, files=files, verify=False, timeout=60)

            r.raise_for_status()
            res_json = r.json()
            if "success" in res_json and res_json["success"] is True:
                print(f"✅ NAS 업로드 성공 → {nas_folder}/{filename}")
                return True
            else:
                print(f"⚠️ NAS 응답 실패: {res_json}")
                raise Exception("NAS returned unsuccessful response")

        except Exception as e:
            print(f"❌ NAS 업로드 실패 ({attempt}/{max_retries}): {e}")
            if attempt < max_retries:
                print(f"⏳ {retry_delay}초 후 재시도합니다...")
                time.sleep(retry_delay)
                attempt += 1
            else:
                print(f"🚨 최대 재시도 횟수 초과 → 업로드 실패: {filename}")
                return False
...
def get_display_id_from_item(item_id: int) -> Optional[str]:
    """1️⃣ item_id → patient_id, 2️⃣ patient_id → display_id 조회"""
    try:
...
#         roi_log = {
#             "video_info": {"fps": fps, "frame_count": frame_count, "sha256": sha256_file(input_path)},
#             "frames": {}
#         }
...
#                     expand_ratio = 0.3  # ✅ ROI 영역 확장 비율 (20~30% 추천)
...
#                         x1, y1 = max(0, x1 - expand_w), max(0, y1 - expand_h)
#                         x2, y2 = min(width, x2 + expand_w), min(height, y2 + expand_h)
        ...
        with tqdm(total=frame_count, desc=f"🔄 {filename} 비식별화 진행중", ncols=110) as pbar:
            for idx in range(frame_count):
                ...
                for r in results:
                    ...
                    expand_ratio = 0.2  # ✅ ROI 영역 확장 비율 (20~30% 추천)
